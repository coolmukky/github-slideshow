...
- **Offline / printable materials:** `offline/` — self-contained HTML you can open with no internet and **Print / Save as PDF** to run the clinic on paper. Start at [`offline/index.html`](./offline/index.html):
  - **★ Integrated package** (`offline/package/`) — the core docs with the **Associate / Professional / Expert** grading woven in: Activity Guide (+ "how you'll be scored"), Workbook (+ a proctor score/level/feedback grid per use case), a **Solution Guide** (the answers — pain/requirement responses + expected products) and a separate **Grading Guide** (rubric, levels, expected-answer-by-level, score grid + feedback). Recommended set for running a graded clinic.
